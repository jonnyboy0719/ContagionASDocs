...
class MarkdownExtra extends \Michelf\Markdown {
	...
	protected function doAnchors($text) {
		...
		// Next, inline-style search: [link text][url "optional title"]
		$text = preg_replace_callback('{
			(				# wrap whole match in $1
			  \[
				(' . $this->nested_brackets_re . ')	# link text = $2
			  ...
			  \[			# literal paren
				...
				(?:
					<(.+?)>	# href = $3
				|
					(' . $this->nested_brackets_re . ')	# href = $4
				)
				[ \n]*
				(			# $5
				  ([\'"])	# quote char = $6
			...
			array($this, '_doAnchors_search_callback'), $text);
	...
	protected function DoPageSearch( $input, $input_title )
	{
		// Grab our json file
		...
		foreach( array_keys($jsonInfo['categories']) as $Categories )
			foreach( array_keys($jsonInfo['categories'][$Categories]) as $CategoryData )
				foreach($jsonInfo['categories'] as $data_key)
				{
					if ( $CategoryData == $input )
						return '<a href="#cat=' . $Categories . '&page=' . $CategoryData . '" rel="page"><span class="type"><i class="fa fa-code"></i> ' . $input_title . '</span></a>';
					if( !empty($data_key[$CategoryData]['functions']) )
						foreach( $data_key[$CategoryData]['functions'] as $ItemKey )
						{
							if ( $ItemKey['name'] == $input )
								return '<a href="#cat=' . $Categories . '&page=' . $CategoryData . '&function=' . $ItemKey['name'] .'" rel="page"><span class="type"><i class="fa fa-code"></i> ' . $input_title . '</span></a>';
						}
				}

		return '<span class="type">' . $input_title . '</span>';
	}

	/**
	 * Callback for search anchors
	 * @param  array $matches
	 * @return string
	 */
	protected function _doAnchors_search_callback($matches) {
		$link_text		=  $this->runSpanGamut($matches[2]);
		$url			=  $matches[3] === '' ? $matches[4] : $matches[3];
		$title			=& $matches[7];
		$attr  = $this->doExtraAttributes("a", $dummy =& $matches[8]);
		...
		$result = $this->DoPageSearch( $url, $link_text );
	...
	protected function doAutoLinks($text) {
		...
		$text = preg_replace_callback('{\[#([^\'">\s]+)\]}i',
			array($this, '_doAutoLinks_search_callback'), $text);
	 ...
	 * @return string
	 */
	protected function _doAutoLinks_search_callback($matches) {
		$url = $this->encodeURLAttribute($matches[1], $text);
		$link = $this->DoPageSearch( $url, $text );
